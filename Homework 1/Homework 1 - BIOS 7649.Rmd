...
cost_size_power_tbl <- kbl(cost_size_power,
  ...
  footnote(general = "$\\delta$ = 1, $\\sigma$ = 0.5, $\\alpha$ = 0.001
...
arraydata <- read.table("C:/Users/domin/Documents/Biostatistics Masters Program/Spring 2024/SMG-BIOS7659/Homework 1/arraydata.txt",stringsAsFactors = F)

sdvalues <- read.table("C:/Users/domin/Documents/Biostatistics Masters Program/Spring 2024/SMG-BIOS7659/Homework 1/sdvalues.txt",stringsAsFactors = F)
...
pb_size_tbl <- kbl(pb_size_df,
                   ...
                   booktabs = TRUE, align = "cc") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  footnote(general = "$\\delta$ = 1, $\\sigma$ = 0.5,FDR = 0.05,$\\pi_0$ = 0.80
           Type = two-sample t-test, Alternative = two-sided")
...
# Calculating sample size for power = 0.80. Size returned is for each group,
# so for entire study multiply by 2. 
lower_size_c <- ceiling(ssize(sd = 0.5,delta = 1, sig.level = 0.001,
                              power = 0.80,alpha.correct = "Bonferonni")) * 2

upper_size_c <- ceiling(ssize(sd = 0.5,delta = 1, sig.level = 0.001,
                              power = 0.95,alpha.correct = "Bonferonni")) * 2

pc_size_df <- data.frame(power = c(0.80,0.95),
                         samplesize = c(lower_size_c,upper_size_c))

pc_size_tbl <- kbl(pc_size_df,
                   caption = "Sample Size Calculation Using ssize",
                   col.names = c("Power","Sample Size"),
                   booktabs = TRUE, align = "cc") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  footnote(general = "$\\delta$ = 1, $\\sigma$ = 0.5, $\\alpha$ = 0.001
           Alpha Correction = None")
